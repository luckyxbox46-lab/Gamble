...
export async function execute(message: Message, args: string[]) {
  ...
  for (let round = 1; round <= rounds; round++) {
    ...
    if (challengerRoll > targetRoll) {
      challengerWins++;
      await message.channel.send(
        `Round ${round}: 🎲 **${challengerRoll}** vs 🎲 ${targetRoll} — **${message.author.username} takes the round!**`
      );
    } else if (targetRoll > challengerRoll) {
      targetWins++;
      await message.channel.send(
        `Round ${round}: 🎲 ${challengerRoll} vs 🎲 **${targetRoll}** — **${target.username} takes the round!**`
      );
    } else {
      await message.channel.send(
        `Round ${round}: 🎲 ${challengerRoll} vs 🎲 ${targetRoll} — **Tie! No point awarded.**`
      );
    }
  }
  ...
  if (challengerWins > targetWins) {
    await message.channel.send(
      `🏆 **${message.author.username} wins the Dice War ${challengerWins}-${targetWins}!** ${target} L + ratio`
    );
  } else if (targetWins > challengerWins) {
    await message.channel.send(
      `🏆 **${target.username} wins the Dice War ${targetWins}-${challengerWins}!** ${message.author} L + ratio`
    );
  } else {
    await message.channel.send(
